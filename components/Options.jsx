...
function Options({ dates, handleChange }) {
    ...
    <Stack alignment="center">
      <Button onClick={togglePopoverActive} icon={CalendarMinor}>
        {selected !== 'custom'
          ...
          : `${format(selectedDates?.start, 'MM/dd/yyyy')} - ${format(selectedDates?.end, 'MM/dd/yyyy')}`}
      </Button>
    </Stack>
  );
  ...
  return (
    <Popover active={popoverActive} activator={activator} onClose={togglePopoverActive} fluidContent fullHeight preferredAlignment="left">
      <div style={{ width: compare ? 1100 : 550 }}>
        <Card>
          <Card.Section>
            <div style={{ display: 'flex', gap: '3rem' }}>
              <FormLayout>
                <Select options={options} value={selected} onChange={(v) => handleChangeSelect(v)} />
                <DatePicker
                  month={month}
                  year={year}
